...
class PostService:
    ...
    async def delete_post(self,post_id:UUID,user_id:UUID):
        ...
        if not post:
            raise CustomException(status_code=status.HTTP_404_NOT_FOUND, message=f"Post with id {post_id} not found")
        if str(post.user_id) != str (user_id):
            raise CustomException(status_code=status.HTTP_403_FORBIDDEN, message=f"Not allowed to delete post with id: {post_id}")
        try:
           ...
           return PostResponse(id=post.id,user_id=post.user_id,title=post.title,content=post.content,created_at=post.created_at,updated_at=post.updated_at)
            ...
            await self.db.rollback()
            raise CustomException(status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, message=f"Something went wrong while deleting post {post_id}: {e}")
